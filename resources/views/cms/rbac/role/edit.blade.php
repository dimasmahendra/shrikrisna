@php
    $title = "Update Role";
    $breadcrumbs[] = ["label" => "Update Role", "url" => route('rbac.role.index')];
    $breadcrumbs[] = ["label" => "Update Role", "url" => "#"];
@endphp
...
<div class="card">
    <div class="card-body px-4 border rounded-4">
        <form action="{{ route('rbac.role.update', [$id]) }}" method="POST" enctype="multipart/form-data">
            {{ csrf_field() }}
            <div class="row">
                ...
                <div class="col-md-8">
                    <div class="pb-2">
                        ...
                        <div class="form-group">
                            ...
                            placeholder="Input Role Name" value="{{ $role->role_name }}" required>
                        </div>
                    </div>
                    <div class="pb-2">
                        <div class="col-md-4">
                        ...
                        <div class="form-group">
                            <div class="form-check form-check-inline">
                                <input class="form-check-input w-30 h-30" type="radio" name="status" id="status1" value="active" {{ ($role->status == "active") ? 'checked' : '' }}>
                                <label class="form-check-label m-t-6 m-l-10" for="status1">Active</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input w-30 h-30" type="radio" name="status" id="status2" value="nonactive" {{ ($role->status == "nonactive") ? 'checked' : '' }}>
                                        ...
                                        <div class="row custom-list-item">
                                            @foreach ($subitem as $index => $item)
                                                <div class="{{ ($key == 'settings') ? 'col-md-4' : 'col-md-3'  }} form-check form-switch m-t-10">
                                                    ...
                                                    {{ (count($permission) > 0 && isset($permission[$item['route_name']]) && $permission[$item['route_name']]) ? 'checked' : ''  }}>
                                                    <label class="form-check-label m-t-5 m-l-11">{{ ucwords($item['text']) }}</label>
                                                </div> 
                                            @endforeach
                                        </div>
                                    </div>
...
@push('js-plugins')
    <script>
        ...
        var sList = "";
        $('.view_checkbox').each(function(i, obj) {
            if (this.checked) {
